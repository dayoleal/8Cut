    ...
    var body: some View {
        HStack {
            HStack {
                ...
                Button(action: { data.insertText(.init("Text"), rect: .init(x: 350, y: 250, width: 100, height: 50)) }) { }
                ...
                Button {
                    ...
                    Image(systemName: "cursorarrow")
                        ...
                        .background(
                            Circle()
                                .foregroundColor(data.isDrawingMode ? Color("mediumGray") : Color.black)
                        )
                }
            }
            .padding(10)
        ...
        .onChange(of: photoItem) { _, newValue in handlePhotoSelection(newValue) }
    }
        ...
        Task {
            if let imageData = try? await item.loadTransferable(type: Data.self),
               let image = UIImage(data: imageData) {
                let rect = CGRect(x: 350, y: 220, width: 150, height: 150)
